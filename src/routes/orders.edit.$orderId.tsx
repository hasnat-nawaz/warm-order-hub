  ...
  if (order.status !== "Pending") {
    return (
      <main className="mx-auto max-w-2xl px-4 py-20 text-center">
        ...
        <Link to="/orders/$orderId" params={{ orderId }} className="mt-4 inline-block text-primary underline">
          Back to order
        </Link>
      </main>
    );
    ...
    >
      ...
      {vendor && (
        <p className="mt-1 text-sm text-muted-foreground">
          Order #{order.id} from <span className="font-semibold text-foreground">{vendor.name}</span>
        </p>
      )}
